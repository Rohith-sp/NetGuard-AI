"use client";
import {
  ResponsiveContainer,
  AreaChart,
  Area,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ReferenceLine,
} from "recharts";
import type { SensorPoint, NodeData } from "../hooks/useLiveData";
import { TempIcon, HumidIcon, LightIcon, SignalIcon } from "./Icons";

// ─── Custom Tooltip ─────────────────────────────────────────────────────────
function ChartTooltip({
  active, payload, label, unit, color,
}: {
  active?: boolean; payload?: { value: number }[]; label?: string; unit: string; color: string;
}) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{
      background: "var(--surface)", border: "1px solid var(--border)",
      borderRadius: "var(--radius)", padding: "8px 12px",
      boxShadow: "var(--shadow-md)", fontFamily: "var(--mono)", fontSize: 12,
    }}>
      <div style={{ color: "var(--text-3)", fontSize: 11, marginBottom: 3 }}>{label}</div>
      <div style={{ color, fontWeight: 600, fontSize: 14 }}>
        {payload[0].value?.toFixed(2)} <span style={{ fontWeight: 400, color: "var(--text-3)" }}>{unit}</span>
      </div>
    </div>
  );
}

// ─── Stat Summary Card ──────────────────────────────────────────────────────
function StatSummary({ label, value, unit, color, icon }: { label: string; value: string; unit: string; color: string; icon: React.ReactNode; }) {
  return (
    <div style={{
      background: "var(--surface2)", border: "1px solid var(--border)",
      borderRadius: "var(--radius)", padding: "14px 16px",
      display: "flex", alignItems: "center", gap: 14, flex: 1,
    }}>
      <div style={{
        width: 40, height: 40, borderRadius: 8,
        background: `color-mix(in srgb, ${color} 12%, transparent)`,
        border: `1px solid color-mix(in srgb, ${color} 25%, transparent)`,
        display: "flex", alignItems: "center", justifyContent: "center",
        flexShrink: 0,
      }}>{icon}</div>
      <div>
        <div style={{ fontSize: 11, color: "var(--text-3)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: 2 }}>{label}</div>
        <div style={{ fontFamily: "var(--mono)", fontSize: 22, fontWeight: 600, color, lineHeight: 1 }}>
          {value} <span style={{ fontSize: 12, fontWeight: 400, color: "var(--text-3)" }}>{unit}</span>
        </div>
      </div>
    </div>
  );
}

// ─── Chart Card Wrapper ─────────────────────────────────────────────────────
function ChartCard({
  title, subtitle, tag, tagColor, children,
}: {
  title: string; subtitle: string; tag: string; tagColor: string; children: React.ReactNode;
}) {
  return (
    <div className="card">
      <div className="card-header">
        <div>
          <div className="card-title">{title}</div>
          <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 2 }}>{subtitle}</div>
        </div>
        <span style={{
          fontSize: 11, fontWeight: 500, padding: "2px 9px", borderRadius: 20,
          background: `color-mix(in srgb, ${tagColor} 10%, var(--surface))`,
          color: tagColor,
          border: `1px solid color-mix(in srgb, ${tagColor} 25%, transparent)`,
        }}>{tag}</span>
      </div>
      <div className="card-body" style={{ paddingTop: 8, paddingBottom: 8 }}>
        {children}
      </div>
    </div>
  );
}

// ─── Empty Placeholder ──────────────────────────────────────────────────────
function EmptyChart({ color, message }: { color: string; message: string }) {
  return (
    <div style={{
      height: 160, display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", gap: 8,
      color: "var(--text-3)", fontSize: 12,
    }}>
      <div style={{
        width: 36, height: 36, borderRadius: "50%",
        background: `color-mix(in srgb, ${color} 10%, var(--surface2))`,
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>
        <SignalIcon width={18} height={18} color={color} />
      </div>
      {message}
    </div>
  );
}

// ─── Main Analytics Tab ─────────────────────────────────────────────────────
export default function AnalyticsTab({
  data, n1, n2,
}: {
  data: SensorPoint[];
  n1: NodeData | undefined;
  n2: NodeData | undefined;
}) {
  // Separate streams
  const tempData     = data.filter(d => d.temp !== undefined);
  const humidData    = data.filter(d => d.humidity !== undefined);
  const lightData    = data.filter(d => d.light !== undefined);

  const latestTemp   = n1?.temp   ?? null;
  const latestHumid  = n1?.humidity ?? null;
  const latestLight  = n2?.light  ?? null;

  const avgTemp   = tempData.length  ? (tempData.reduce((s, d) => s + (d.temp ?? 0), 0)  / tempData.length).toFixed(1)  : "—";
  const avgHumid  = humidData.length ? (humidData.reduce((s, d) => s + (d.humidity ?? 0), 0) / humidData.length).toFixed(1) : "—";
  const minLight  = lightData.length ? Math.min(...lightData.map(d => d.light ?? 0)) : null;
  const maxLight  = lightData.length ? Math.max(...lightData.map(d => d.light ?? 0)) : null;

  const n1Online  = n1?.online ?? false;
  const n2Online  = n2?.online ?? false;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>

      {/* ── KPI Summary Row ───────────────────────────────────────────────── */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        <StatSummary
          icon={<TempIcon width={20} height={20} color="var(--red)" />} label="Temperature" unit="°C" color="var(--red)"
          value={latestTemp !== null ? latestTemp.toFixed(1) : "—"}
        />
        <StatSummary
          icon={<HumidIcon width={20} height={20} color="var(--blue)" />} label="Humidity" unit="%" color="var(--blue)"
          value={latestHumid !== null ? latestHumid.toFixed(1) : "—"}
        />
        <StatSummary
          icon={<LightIcon width={20} height={20} color="var(--amber)" />} label="Light Intensity" unit="LUX" color="var(--amber)"
          value={latestLight !== null ? String(latestLight) : "—"}
        />
      </div>

      {/* ── Node Status Strip ─────────────────────────────────────────────── */}
      <div style={{ display: "flex", gap: 14 }}>
        {[
          { id: "Node A · DHT11",  topic: "netguard_rohit_77/device1", online: n1Online, color: "var(--green)",  desc: "Temperature & Humidity" },
          { id: "Node B · LDR",    topic: "netguard_rohit_77/device2", online: n2Online, color: "var(--amber)",  desc: "Ambient Light" },
        ].map(node => (
          <div key={node.id} style={{
            flex: 1, background: "var(--surface)", border: "1px solid var(--border)",
            borderRadius: "var(--radius)", padding: "12px 16px",
            display: "flex", alignItems: "center", gap: 12,
            boxShadow: "var(--shadow)",
          }}>
            <div style={{
              width: 8, height: 8, borderRadius: "50%",
              background: node.online ? "var(--green)" : "var(--border2)",
              boxShadow: node.online ? "0 0 6px var(--green)" : "none",
              flexShrink: 0,
              animation: node.online ? "livepulse 2s infinite" : "none",
            }} />
            <div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 500, color: "var(--text)" }}>{node.id}</div>
              <div style={{ fontSize: 11, color: "var(--text-3)" }}>{node.topic} · {node.desc}</div>
            </div>
            <span style={{
              marginLeft: "auto", fontSize: 10.5, fontWeight: 500,
              padding: "2px 8px", borderRadius: 20,
              background: node.online ? "var(--green-bg)" : "var(--surface2)",
              color: node.online ? "var(--green)" : "var(--text-3)",
              border: `1px solid ${node.online ? "var(--green-mid)" : "var(--border)"}`,
            }}>{node.online ? "LIVE" : "OFFLINE"}</span>
          </div>
        ))}
      </div>

      {/* ── Temperature Chart ─────────────────────────────────────────────── */}
      <ChartCard
        title="Temperature"
        subtitle={`Session avg: ${avgTemp}°C · ${tempData.length} readings · Last ${tempData.length > 0 ? tempData.at(-1)?.t : "—"}`}
        tag={latestTemp !== null ? `${latestTemp.toFixed(1)} °C` : "No data"}
        tagColor="var(--red)"
      >
        {tempData.length < 2 ? (
          <EmptyChart color="var(--red)" message="Waiting for temperature readings from Node A…" />
        ) : (
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={tempData} margin={{ top: 6, right: 8, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="gradTemp" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%"   stopColor="#dc2626" stopOpacity={0.18} />
                  <stop offset="95%"  stopColor="#dc2626" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="t" tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} interval="preserveStartEnd" axisLine={false} tickLine={false} />
              <YAxis domain={["auto", "auto"]} tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip unit="°C" color="var(--red)" />} />
              <Area
                type="monotone" dataKey="temp" stroke="#dc2626" strokeWidth={2}
                fill="url(#gradTemp)" dot={false} activeDot={{ r: 4, fill: "#dc2626" }}
                isAnimationActive={false}
              />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </ChartCard>

      {/* ── Humidity Chart ────────────────────────────────────────────────── */}
      <ChartCard
        title="Relative Humidity"
        subtitle={`Session avg: ${avgHumid}% · ${humidData.length} readings · DHT11 sensor (±5% accuracy)`}
        tag={latestHumid !== null ? `${latestHumid.toFixed(1)} %` : "No data"}
        tagColor="var(--blue)"
      >
        {humidData.length < 2 ? (
          <EmptyChart color="var(--blue)" message="Waiting for humidity readings from Node A…" />
        ) : (
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={humidData} margin={{ top: 6, right: 8, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="gradHumid" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%"  stopColor="#2563eb" stopOpacity={0.15} />
                  <stop offset="95%" stopColor="#2563eb" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="t" tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} interval="preserveStartEnd" axisLine={false} tickLine={false} />
              <YAxis domain={[0, 100]} tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip unit="%" color="var(--blue)" />} />
              <ReferenceLine y={60} stroke="#2563eb" strokeDasharray="4 3" strokeOpacity={0.35} label={{ value: "60%", fill: "var(--text-3)", fontSize: 9, fontFamily: "var(--mono)" }} />
              <Area
                type="monotone" dataKey="humidity" stroke="#2563eb" strokeWidth={2}
                fill="url(#gradHumid)" dot={false} activeDot={{ r: 4, fill: "#2563eb" }}
                isAnimationActive={false}
              />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </ChartCard>

      {/* ── Light Intensity Chart ─────────────────────────────────────────── */}
      <ChartCard
        title="Ambient Light Intensity"
        subtitle={`Range: ${minLight ?? "—"} – ${maxLight ?? "—"} LUX · ${lightData.length} readings · LDR sensor`}
        tag={latestLight !== null ? `${latestLight} LUX` : "No data"}
        tagColor="var(--amber)"
      >
        {lightData.length < 2 ? (
          <EmptyChart color="var(--amber)" message="Waiting for light readings from Node B…" />
        ) : (
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={lightData} margin={{ top: 6, right: 8, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="gradLight" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%"  stopColor="#d97706" stopOpacity={0.18} />
                  <stop offset="95%" stopColor="#d97706" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="t" tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} interval="preserveStartEnd" axisLine={false} tickLine={false} />
              <YAxis domain={["auto", "auto"]} tick={{ fontFamily: "var(--mono)", fontSize: 9, fill: "var(--text-3)" }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip unit="LUX" color="var(--amber)" />} />
              <Area
                type="monotone" dataKey="light" stroke="#d97706" strokeWidth={2}
                fill="url(#gradLight)" dot={false} activeDot={{ r: 4, fill: "#d97706" }}
                isAnimationActive={false}
              />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </ChartCard>

    </div>
  );
}
